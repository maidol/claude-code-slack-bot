...
## Routing requests through a custom API gateway / proxy

`.env` 里写 `ANTHROPIC_API_KEY` 和 `ANTHROPIC_BASE_URL` 是**不够的**。Claude CLI 启动时会读 `~/.claude/settings.json` 并用其中的 `env` 和 `apiKeyHelper` 覆盖父进程传入的环境变量。再加上某些插件（如 `claude-mem`、`claude-code-router`）会通过 hook 机制把 CLI 出站请求拦截到本地 sidecar。这两层叠加，bot 的 `.env` 网关设置很容易被绕过。

### 推荐方案：项目级 `.claude/settings.local.json`
...
2. 填入网关凭据：
   ...
   {
     "env": {
       "ANTHROPIC_API_KEY": "sk-proxy-gateway-v1-XXXX",
       "ANTHROPIC_BASE_URL": "https://your-gateway.example.com"
     },
     "apiKeyHelper": "echo 'sk-proxy-gateway-v1-XXXX'"
   }
   ```
3. `pm2 restart claude-slack-bot`

`settings.local.json` 在 `.gitignore` 中，**永远不会提交**。`.example` 文件可以 commit 作为模板。

### ⚠️ 重要限制 1：cwd 决定 settings.json 是否被加载

Claude CLI 按 **cwd 向上查找** `.claude/settings.json`。bot 派生 CLI 时 cwd 是用户通过 `-cwd` 设置的工作目录。

| Bot 查询的 cwd | 项目级 `.claude/settings.local.json` 是否生效 |
|---------------|--------------------------------------------|
| 本 repo 内（`/home/.../claude-code-slack-bot/...`） | ✅ 生效 |
| `BASE_DIRECTORY` 下任一子项目 | ❌ 不生效，除非 `BASE_DIRECTORY` 父目录也放 `.claude/` |
| 其他绝对路径 | ❌ 不生效 |

**实际中 bot 跑查询几乎都不在本 repo**——这意味着仅靠项目级 settings 几乎不能覆盖 bot 的真实使用场景。

完整覆盖方案：把 `.claude/settings.local.json` 放在 `BASE_DIRECTORY` 父目录（或更上层），覆盖所有可能的 cwd。例如 `BASE_DIRECTORY=/home/mark/cbot/` 时，建议放在 `/home/mark/.claude/settings.json`（用户级）或 `/home/mark/cbot/.claude/settings.local.json`（覆盖所有子项目）。

### ⚠️ 重要限制 2：本地插件拦截

如果机器上安装了以下任一插件/工具，它们会**先于 settings.json** 接管 CLI 出站流量：

- **claude-mem**（plugin daemon，监听 `127.0.0.1:37777`）：通过 `SessionStart` / `SessionEnd` hook 拦截，所有 CLI 请求实际打到这个 sidecar，再由它转发到上游
- **claude-code-router**（监听 `127.0.0.1:3456`）：独立 router，通过自己的配置文件决定上游 provider

排查方法：
```bash
# 看 CLI 实际请求目标 host
echo "say hi" | strace -f -e trace=sendto -s 512 -o /tmp/c.log claude -p --model haiku
grep -oE 'Host: [^\\]+' /tmp/c.log | sort -u
```

- 输出 `Host: api.anthropic.com` 或 `Host: your-gateway.example.com` → settings.json 生效
- 输出 `Host: 127.0.0.1:37777`（claude-mem）或 `Host: 127.0.0.1:3456`（router）→ 被本地 sidecar 接管，settings.json 无效

如果被接管想绕开，三种方法：
1. 禁用相关插件：`pm2 stop` 该 daemon，或在 `~/.claude/settings.json` 的 `enabledPlugins` 里把 `claude-mem@thedotmack` 改为 `false`
2. 改用 `--bare` flag 启动 CLI（跳过 hooks），需要修改 `src/cli-handler.ts` 的 `args`
3. 直接修改 sidecar 配置（如 `~/.config/claude-code-router/config.json`），把网关写进去

### ⚠️ 重要限制 3：`apiKeyHelper` 优先级最高

`~/.claude/settings.json` 里如果存在 `apiKeyHelper`，它返回的值会**覆盖**项目级 `env.ANTHROPIC_API_KEY`。所以项目级 settings 通常需要**同时设置 env 和 apiKeyHelper**（两者都指向网关 key），才能完全压住用户级配置。

### 验证流程

```bash
# 1) 用项目 cwd 直接跑 claude，确认 settings 是否生效
cd /home/.../claude-code-slack-bot
echo "say hi" | claude -p --model haiku

# 2) 抓 host header
echo "say hi" | strace -f -e trace=sendto -s 512 -o /tmp/c.log claude -p --model haiku
grep -oE 'Host: [^\\]+' /tmp/c.log | sort -u

# 3) 实际触发 bot（在 Slack 发消息），看 pm2 logs 里的 spawn cwd 和 host
pm2 logs claude-slack-bot --lines 100
```
